    ...
    if ($pseudo != "" && $email != "" && $password != "") {
        if ($password == $password2) {
            ...
            if (UpdateUser($_SESSION['idUtilisateur'], $pseudo, $password, $email)) {
                //Si la requète UpdateUser réussi, on remplace toutes ses infos
            ...
            $message .= "<div class='alert alert-danger' role='alert'>Les 2 mots de passe ne correspondent pas. Veuillez réessayer.</div>";
        }
    } else {
        //Sinon, on demande à l'utilisateur de remplir tous les champs
...
<body>
        ...
        <div class="d-flex bd-highlight justify-content-center" style="padding:3%;">
            <form class="row g-3" method="post" action="#">
                ...
                <div>
                    ...
                    <input type="password" class="form-control" placeholder="<?php echo ($_SESSION["password"]) ?>" name="password2">
                </div>
                <div>
    ...
    <div class="border border-danger" style="margin-top: 5%; margin-left: 20%; margin-right:20%; max-height: 700px; overflow-y: auto; overflow-x: hidden;">
        ...
        <div class="row d-flex bd-highlight justify-content-center" style="padding:3%;">
            <?php
            //Récupère toutes les infos de chaque film de la vidéothèque de l'utilisateur
            $favoris = GetAllUserFavoriteFilm($_SESSION['idUtilisateur']);
            //Pour chaque films récupéré, on va prendre leur ID 
            foreach ($favoris as $f) {
                $film = GetFilmById($f['idFilm']);
                //Puis on va créer des cartes avec : leur image, leur titre et un bouton pour les retirer de la vidéothèque
            ?>
                <div class="col">
                    <div class="card" style="width: 18rem;">
                        ...
                        <div class="card-body">
                            ...
                            <a class="btn btn-danger" style="width: 100%; float: right; padding: 5%;" href="profil.php?favoris=2&idFilm=<?= $f['idFilm'] ?>&favoris=1">
        ...
        <div class="d-flex bd-highlight justify-content-center" style="padding:3%;">
            <table class="table table-sm table-dark table-hover">
                ...
                <tbody>
                    ...
                    //Puis pour chaque post correspondant, on les ajouter à un tableau qui sera notre liste de critiques
                    foreach ($posts as $p) {
                    ?>
                        <tr class="table-light">
                            ...
                            <td>
                                <?php
                                //On affiche l'état de la critique selon la variable estValide (0 = en cours, 1 = acceptée, 2 = refusée)
                                switch ($p['estValide']) {
                                    case '0':
