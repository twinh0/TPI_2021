...
if (isset($_POST["submitButton"])) {
    ...
    if ($titre != "" && $contenu != "" && $note != "") {
        ...
        if (InsertNewCritique($titre, $date, $contenu, $note, $_SESSION['idUtilisateur'])) {
            $message .= "<div class='alert alert-success' role='alert'>Critique envoyée. Un administrateur va vérifier votre critique avant de la valider.</div>";
        } else {
            //Sinon, on envoie une erreur
            $message = "<div class='alert alert-danger' role='alert'>La critique n'as pas pu être envoyée.</div>";
        }
    } else {
        //Sinon, on demande de remplir tous les champs
            ...
            <form class="row g-3" method="post" action="#">
                <div>
                    ...
                    <select class="form-control" name="titre">
                        ...
                        <?php
                        //Récupère les infos de chaque films
                        $films = GetAllFilms();
                        //Puis on ajoute en option du select chaque film de la liste 
                        foreach ($films as $f) {
