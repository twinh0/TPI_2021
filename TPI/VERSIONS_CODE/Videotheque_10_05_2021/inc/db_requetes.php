...
//CREATION CRITIQUE
//Fonction qui importe la critique
function insertNewCritique($titre, $date, $contenu, $note, $estValide)
{
    connexion()->setAttribute(PDO::ATTR_EMULATE_PREPARES, TRUE);
    //Ajoute une critique avec id de base, sa date, son contenu, sa note, et si elle est valide
    $poste = connexion()->prepare('INSERT INTO critique VALUES (null,"' . $date . '","' . $contenu . '","' . $note . '","' . $estValide . ")");
    if ($poste->execute()) {
        return true;
    } else {
        return false;
