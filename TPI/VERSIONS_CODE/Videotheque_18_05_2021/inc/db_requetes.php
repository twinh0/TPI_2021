...
function UpdateCritqiue($idCritique, $titre, $date, $contenu, $note, $idutilisateur)
{
    ...
    if ($ps == null) {
        $ps = Videotheque()->prepare('UPDATE `critique` SET `dateCritique` = :dateCritique, `contenu` = :contenu, `note` = :note, `estValide` = :estValide, `idFilm` = :idFilm WHERE (`idCritique` = :idCritique)');
    }
    $answer = false;
    try {
        $ps->bindParam(':dateCritique', $date, PDO::PARAM_STR);
        ...
        $ps->bindParam(':idFilm', $titre, PDO::PARAM_INT);
        $ps->bindParam(':idUtilisateur', $idutilisateur, PDO::PARAM_INT);
        $ps->bindParam(':idCritique', $idCritique, PDO::PARAM_INT);
